// src/screens/businessProfile/BusinessStep1.tsx
...
import { BusinessProfileStackParamList } from '../../navigation/BusinessProfileNavigator'; // Import the param list for type safety [cite: src/navigation/BusinessProfileNavigator.tsx]
// No need to import auth/db here if only navigating

type BusinessProfileStep1ScreenProps = NativeStackScreenProps<BusinessProfileStackParamList, 'BusinessProfileStep1'>;

export default function BusinessProfileStep1Screen({ navigation }: BusinessProfileStep1ScreenProps) {
  ...
  const handleCompleteStep = async () => {
    ...
    try {
        ...
        // Navigate to the next step in the BusinessProfileNavigator
        ...
        console.error("Error completing business profile step 1:", error);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        >
          {loading ? <ActivityIndicator animating={true} color={theme.colors.onPrimary} size="small" /> : 'השלם שלב 1 (מנהל)'}
        </PaperButton>
      </View>
    </SafeAreaView>
  );
